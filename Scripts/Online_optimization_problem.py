...
# Sort the dataset by start time to handle events sequentially
time_events = sorted(set(pdu_data['start']).union(set(pdu_data['end'])))

# Iterate over each time event
for tau in time_events:
    # Determine active PDUs at the current time instance
    Aq = list(pdu_data[(pdu_data['start'] <= tau) & (pdu_data['end'] >= tau)]['id'])

    # Check if the current event is a PDU generation (new arrival)
    new_pdus = pdu_data[pdu_data['start'] == tau]['id'].tolist()
    if new_pdus:
        # Set Uq: UPF instances that are active at the current time instance
        active_upfs = {result['UPF_instance'] for result in results if
                       result['Time'] <= tau and result['UPF_active'] == 1}
        Uq = list(active_upfs)
        ...
        # Define Sets
        model.Aq = Set(initialize=Aq, ordered=True)  # Set of active PDU sessions at current time
        ...
        model.y = Var(model.Aq, model.U,
                      within=Binary)  # Binary indicating if PDU session j is anchored to UPF instance i
        ...
        def upf_activation_constraint(model, i):
            return model.x[i] >= model.y[new_pdus[0], i]  # Only enforce for new PDU session arriving
        ...
        def upper_bound_cpu_constraint(model, j, i):
            return model.s_total[i] <= M2 * (1 - model.y[j, i])
        ...
        for i in model.U:
            results.append({
                'PDU_session': new_pdus[0],  # only the newly arrived PDU session
                'UPF_instance': i,
                'Time': tau,
    ...
    # If there are no new PDU sessions (only terminations), update active UPFs without optimization
